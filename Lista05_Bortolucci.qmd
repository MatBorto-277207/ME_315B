...
```{sql connection=con}
SELECT PNAME, ACRES, 1.2 * ACRES AS NOVA_AREA  FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES/3 AS AREA_AJUSTADA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (50 + FEE)/2 AS TAXA_AJUSTADA FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT PNAME) AS NOMES_DIFERENTES, 
        COUNT(PNAME) AS QUANTIDADE_DE_NOMES FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(0.02 * ACRES) AS MEDIA_FEE FROM PRESERVE
WHERE STATE = "AZ"
```
...
```{sql connection=con}
SELECT ENO, ENAME, SALARY, 1.1 * SALARY AS NEW_SALARY FROM EMPLOYEE
ORDER BY NEW_SALARY
```
...
```{sql connection=con}
SELECT COUNT(DNO) AS DNO_20 FROM EMPLOYEE
WHERE DNO = 20
```
